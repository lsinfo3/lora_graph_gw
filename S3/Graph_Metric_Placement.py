...
def create_placement(vars):
    ...
    for g in Gateways:
        latlon=utm.to_latlon(Sensors[g][0],Sensors[g][1],56,"H")
        output['id'].append(g)
        output['x'].append(latlon[1])
        output['y'].append(latlon[0])
        output['height'].append(5.0)
        output['environment'].append("urban")
    pd.DataFrame(data=output).to_csv('gateways_Placement_Sydney.csv')
    ...
    pd.DataFrame(data=output).to_csv('reachable_sensors_Sydney.csv')
    pd.DataFrame(data=output).to_json('reachable_sensors_Sydney.json')


if __name__ == "__main__":
    base_range=hata(8,15,1)
    Sensors=[]
