...
def create_placement(vars):
    ...
    for g in Gateways:
        latlon=utm.to_latlon(Sensors[g][0],Sensors[g][1],30,"U")
        output['id'].append(g)
    ...
    for sens in Sensors_to_export:
        if(sens[2]!=0):
            latlon=utm.to_latlon(sens[0],sens[1],30,"U")
...
def run_clustering(num_clusters,num_threads,sensor_file,run):
    base_range=hata(12,15,1)
    Sensors=[]
    #num_clusters=2
    #num_threads=2 #only values smaller than or equal to num_clusters improve compute time
    with open('sensors_'+str(sensor_file)+'.csv', newline='') as csvfile:
    ...
    pd.DataFrame(data=output).to_csv('gateways_Placement_london_'+str(sensor_file)+'_Cluster_'+str(num_clusters)+'_run_'+str(run)+'.csv')
    pd.DataFrame(data=outputsens).to_csv('reachable_sensors_london_'+str(sensor_file)+'_Cluster_'+str(num_clusters)+'_run_'+str(run)+'.csv')
    pd.DataFrame(data=outputsens).to_json('reachable_sensors_london_'+str(sensor_file)+'_Cluster_'+str(num_clusters)+'_run_'+str(run)+'.json')

if __name__ == "__main__":
    num_threads=4
    Sensor_File_Names=['london']
    configurations=[4,8,12,16,20,25,30,35,40]
    output={"configurations": [], "runtimes": []}
    start=datetime.now()
    run_clustering(1,1,'london',0)
    output["runtimes"].append(str(datetime.now()-start))
    output["configurations"].append(1)
    #print(output)
    df=pd.DataFrame.from_dict(output)
    df.to_csv("runtimes.csv",index=False,header=True)
    for run in range(10):
        for i in configurations:
            for j in Sensor_File_Names:
                start=datetime.now()
                run_clustering(i,min(num_threads,i),j,run)
                output["runtimes"].append(str(datetime.now()-start))
