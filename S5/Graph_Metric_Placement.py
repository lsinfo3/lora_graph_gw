...
#nur euklidische Koordinaten als Integer
#def generateEdges(Sensors,Max_Range):
#    nodes=Sensors
#    edges=[[]for i in nodes]
#    for curr_sensor in Sensors:
#        minreachx=curr_sensor[0]-Max_Range
#        if(minreachx<0):
#            minreachx=0
#        maxreachx=curr_sensor[0]+Max_Range
#        minreachy=curr_sensor[1]-Max_Range
#        if(minreachy<0):
#            minreachy=0
#        maxreachy=curr_sensor[1]+Max_Range
#        for x in range(minreachx,maxreachx+1):
#            for y in range(minreachy,maxreachy+1):
#                if([x,y] in nodes and [x,y]!=curr_sensor):
#                    if(calculateDistance(curr_sensor[0],x,curr_sensor[1],y)<=Max_Range):
#                        edges[nodes.index(curr_sensor)].append(nodes.index([x,y]))
#    return(edges)
...
def create_placement(vars):
    ...
    output = {"lon": [], "lat": [], "BestGW": [],"SF": [], "NumberOfSensors": []}
    ...
    with open("sensors.csv", newline='') as csvfile:
        ...
        for curr in data:
            try:
                ...
                pass
    create_placement([base_range,Sensors])
