...
def run_clustering(num_clusters,num_threads,filename,run):
    ...
    pd.DataFrame(data=output).to_csv('gateways_Placement_'+str(filename)+'_Cluster_'+str(num_clusters)+'_run_'+str(run)+'.csv')
    pd.DataFrame(data=outputsens).to_csv('reachable_sensors_'+str(filename)+'_Cluster_'+str(num_clusters)+'_run_'+str(run)+'.csv')
    pd.DataFrame(data=outputsens).to_json('reachable_sensors_'+str(filename)+'_Cluster_'+str(num_clusters)+'_run_'+str(run)+'.json')

if __name__ == "__main__":
    num_threads=14
    filenames=['sensors_circle_'+str(i) for i in range(10)]
    filenames+=['sensors_wrong_circle_'+str(i) for i in range(10)]
    filenames+=['sensors_random_square_'+str(i) for i in range(10)]
    filenames.append('sensors_uniform')
    print(filenames)
    ...
    for run in range(10):
        for name in filenames:
            for i in configurations:
                start=datetime.now()
                run_clustering(i,min(num_threads,i),name,run)
