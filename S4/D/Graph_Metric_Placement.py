...
def create_placement(vars):
    ...
    pd.DataFrame(data=output).to_csv('gateways_Placement_Wuerzburg.csv')
    ...
    pd.DataFrame(data=output).to_csv('reachable_sensors_Wuerzburg.csv')
    pd.DataFrame(data=output).to_json('reachable_sensors_Wuerzburg.json')
